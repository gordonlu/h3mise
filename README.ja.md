...
1. **作品を定義する**: ログライン、本文、総尺を整理し、ストーリーを撮影可能な StoryBeat に分割します。内蔵の物語骨格（叙事骨架）から始めることもでき、結果はすべて手動で編集できます。
2. **任意の Storyboard**: まず無料の 3 / 6 / 9 コマ文字プランを確認し、その後で独立した RunningHub 生図 App を使うか決めます。長いストーリーは自動でページ分割され、ページごと・単コマ再生成ごとに費用を個別確認します。
...
- **任意の Storyboard**: 物語セグメント数に応じた 3 / 6 / 9 コマ文字プラン。9 コマ超は自動ページ分割。文字編集は無料で、全ページまたは単コマの生図は明示確認後にのみ RunningHub を呼びます。固定黒枠・分割・再構成はローカル FFmpeg で行い、単コマ再生成はバージョンを保持します。
- **物語骨格**: 反転、誤解の深刻化、ルール崩壊、カウントダウン、情報格差、感情回収、遅延する証拠、権限差し違えなど、3 / 6 / 9 セグメント変種を内蔵。テーマ入力で AI が順位付けし、AI 未設定や失敗時はローカルマッチングを自動利用します。
...
- **ストーリー**: ストーリー事実、本文、総尺、StoryBeat 管理。任意の 3 / 6 / 9 セグメント骨格は AI なしでも動作します。
- **Storyboard（任意）**: 文字コマの無料編集。ページごとの有料生図、単コマ再生成、バージョン保持、承認を各操作ごとに確認。丸ごとスキップしても Shot / Take パイプラインを妨げません。
