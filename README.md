...
1. **定义作品**：整理梗概、正文和总时长，把故事拆成可拍摄的 StoryBeats；也可从内置叙事骨架开始，所有结果仍可手工修改。
2. **可选 Storyboard**：先免费检查 3 / 6 / 9 格文字规划，再决定是否调用独立的 RunningHub 生图 App。长故事会自动分页，每页和单格重生都单独确认费用。
...
- **可选 Storyboard**：按叙事段数生成 3 / 6 / 9 格文字规划，超过 9 段自动分页；文字编辑免费，整页或单格生图均明确确认后才调用 RunningHub。固定黑边框、拆格和重组由本地 FFmpeg 完成，单格重生保留版本。
- **叙事骨架**：内置反转、误会升级、规则失控、倒计时、信息差、情绪回收、证据延迟和权力错判等节奏结构，提供 3 / 6 / 9 段变体。输入主题后可由 AI 排序；未配置 AI 或调用失败时自动使用本地匹配。
...
- **故事**：故事事实、正文、总时长与 StoryBeat 管理；可选用 3 / 6 / 9 段叙事骨架，AI 不可用时仍能本地推荐和套用。
- **Storyboard（可选）**：免费编辑文字分格；按页确认付费生图、单格重生、版本保留与批准。可以完全跳过，不阻塞现有 Shot / Take 链路。
