...
10. 多人镜头必须有清楚的焦点接力。三个或以上角色先后行动或说话时，不得让所有人长期等权挤在远景中；先用短暂构图交代空间，再用一个连续、可执行的主导镜头运动跟随当前动作或说话者。运动方向必须来自既有站位和画面方位；若用户指定从左向右，就按左到右依次平移或跟随重新构图。全镜头只保留一种主导运动，不来回摇摆，不凭空新增位置。
...
12. 剧情信息不可删减：若当前提示词含 Episode premise、Current story beat、Narrative change、Spoken lines、Narrative focus sequence 或 Storytelling priority，必须完整保留并放在 detailed_description 前部。Current story beat 是本镜头事件边界；不得把它压缩成单纯的场景、姿势或摄影描述。
13. 多人焦点接力：若提示词含 Narrative focus sequence，必须让构图依次突出当前角色。角色横向排列且方位已知时，可用一次连续平移依次到达焦点人物；不得把三人始终放在同一个遥远全景里。若已有明确 fixed camera 锁定，不得暗中制造景别变化，应保留冲突供用户核对。
